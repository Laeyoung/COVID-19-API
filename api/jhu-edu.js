...
router.get("/latest", function (req, res) {
  ...
  let latest = onlyCountries
    ? responseSet.latestOnlyCountries
    : responseSet.latest;
...
router.get("/timeseries", function (req, res) {
  ...
  let timeseries = onlyCountries
    ? responseSet.timeseriesOnlyCountries
    : responseSet.timeseries;
...
async function updateCSVDataSet() {
  ...
  try {
    await Promise.all(
      Object.entries(csvPath).map(([category, path]) =>
        queryCsvAndSave(dataSource, path, category),
      ),
    );
    ...
    for (const [category, value] of Object.entries(dataSource)) {
      for (const [name, item] of Object.entries(value)) {
        ...
        for (const date of keys.slice(4)) {
          ...
          nestedProperty.set(
            timeseries[name],
            `timeseries.${fixedDatePropertyName}.${category}`,
            Number(item[date]),
          );
        }
      }
...
function createPropertyIfNeed(target, name, item) {
  if (!nestedProperty.has(target, name)) {
    ...
    const countryCode =
      ...
      lookup.byCountry(fixedCountryCodes[countryName]);
    if (countryCode) {
